...
update:
  ...
  cmpeqi r6, 0 ; compare if r6 (active register) is equal to zero
  bf inactive  ; branch if true to inactive
  ...
  bt update    ; branch if true to update (there is no relative jmp instruction, so relative branch is used instead)

inactive:
  stf r6, 0x2b ; store r6 (zero) to FU MD (Memory Data) word - write to PWM duty cycle register
  btsti r1, 0  ; always true (for unconditional relative branch)
  bt update    ; branch if true to update (there is no relative jmp instruction, so relative branch is used instead)
...
alldone:
  ...
  bt update    ; branch if true to update
